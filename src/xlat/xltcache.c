...
static size_t xlat_new_cache_size = XLAT_NEW_CACHE_SIZE;
#ifdef XLAT_GENERATIONAL_CACHE
static size_t xlat_temp_cache_size = XLAT_TEMP_CACHE_SIZE;
static size_t xlat_old_cache_size = XLAT_OLD_CACHE_SIZE;
#endif

static inline size_t parse_mb_env(const char *name, size_t fallback_bytes)
{
    const char *env = getenv(name);
    if( env == NULL || *env == '\0' ) return fallback_bytes;
    long mb = strtol(env, NULL, 10);
    if( mb <= 0 ) return fallback_bytes;
    if( mb < 4 ) mb = 4;
    if( mb > 1024 ) mb = 1024;
    return (size_t)mb * (size_t)1024 * (size_t)1024;
}
...
void xlat_cache_init(void) 
{
    if( !xlat_initialized ) {
        ...
        const char *metrics = getenv("LXDREAM_JIT_METRICS");
        if( metrics && *metrics ) {
            fprintf(stderr, "[JIT] metrics enabled (env LXDREAM_JIT_METRICS)\n");
        }
        /* Allow env overrides */
        xlat_new_cache_size = parse_mb_env("LXDREAM_JIT_NEW_MB", XLAT_NEW_CACHE_SIZE);
#ifdef XLAT_GENERATIONAL_CACHE
        xlat_temp_cache_size = parse_mb_env("LXDREAM_JIT_TEMP_MB", XLAT_TEMP_CACHE_SIZE);
        xlat_old_cache_size = parse_mb_env("LXDREAM_JIT_OLD_MB", XLAT_OLD_CACHE_SIZE);
#endif
...
void xlat_flush_cache() 
{
    ...
    xlat_new_cache_ptr->size = (int)(xlat_new_cache_size - 2*sizeof(struct xlat_cache_block));
    ...
    xlat_temp_cache_ptr->size = (int)(xlat_temp_cache_size - 2*sizeof(struct xlat_cache_block));
    tmp = NEXT(xlat_temp_cache_ptr);
    tmp->active = 1;
    tmp->size = 0;
    xlat_old_cache_ptr = xlat_old_cache;
    xlat_old_cache_ptr->active = 0;
    xlat_old_cache_ptr->size = (int)(xlat_old_cache_size - 2*sizeof(struct xlat_cache_block));
...
gboolean xlat_is_code_pointer( void *p )
{
    ...
    xlat_cache_block_t block = XLAT_BLOCK_FOR_CODE(p);
    if( (((char *)block) - (char *)xlat_new_cache) < (ptrdiff_t)xlat_new_cache_size ) {
         /* Pointer is in new cache */
        region = (char *)xlat_new_cache;
        region_size = xlat_new_cache_size;
    }
#ifdef XLAT_GENERATIONAL_CACHE
    else if( (((char *)block) - (char *)xlat_temp_cache) < (ptrdiff_t)xlat_temp_cache_size ) {
         /* Pointer is in temp cache */
        region = (char *)xlat_temp_cache;
        region_size = xlat_temp_cache_size;
    } else if( (((char *)block) - (char *)xlat_odl_cache) < (ptrdiff_t)xlat_old_cache_size ) {
...
{
    xlat_check_cache_integrity( xlat_new_cache, xlat_new_cache_ptr, (int)xlat_new_cache_size );
#ifdef XLAT_GENERATIONAL_CACHE
    xlat_check_cache_integrity( xlat_temp_cache, xlat_temp_cache_ptr, (int)xlat_temp_cache_size );
    xlat_check_cache_integrity( xlat_old_cache, xlat_old_cache_ptr, (int)xlat_old_cache_size );
#endif
}
