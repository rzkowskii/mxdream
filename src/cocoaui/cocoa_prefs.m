...
//        [view scrollRectToVisible: NSMakeRect(0,0,1,1)];
        ...
        /* Apply any runtime video toggles when the pane is created */
        if( group == lxdream_get_config_group(CONFIG_GROUP_VIDEO) ) {
            extern void cocoa_apply_video_runtime_toggles(void);
            cocoa_apply_video_runtime_toggles();
        }
    }
    return self;
}
...
- (id)initWithContentRect:(NSRect)contentRect 
{
    if( [super initWithContentRect: contentRect
        ...
        config_panes[2] = [[LxdreamPrefsPane alloc] initWithFrame: NSMakeRect(0,0,600,400)
                             ...
                             scrollable: YES];
        /* Video pane */
        config_panes[3] = [[LxdreamPrefsPane alloc] initWithFrame: NSMakeRect(0,0,600,260)
                              title: NS_("Video")
                              configGroup: lxdream_get_config_group(CONFIG_GROUP_VIDEO)
                              scrollable: NO];
...
- (void) initToolbar
{
    ...
    NSToolbarItem *video=[self createToolbarItem: @"Video" label: @"Video"
                            tooltip: @"Video settings" icon: @"tb-ctrls"
                            action: @selector(video_action:)];
    toolbar_ids = [NSArray arrayWithObjects: @"Paths", @"Controllers", @"Hotkeys", @"Video", nil ];
    toolbar_defaults = [NSArray arrayWithObjects: @"Paths", @"Controllers", @"Hotkeys", @"Video", nil ];
    NSArray *values = [NSArray arrayWithObjects: paths, ctrls, hotkeys, video, nil ];
...
- (void)video_action: (id)sender
{
    [self setContentView: config_panes[3]];
}
...
{
    return [NSArray arrayWithObjects: @"Paths", @"Controllers", @"Hotkeys", @"Video", nil ];
}
