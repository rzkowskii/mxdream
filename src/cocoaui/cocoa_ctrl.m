    ...
    if( new_device_idx == LOAD_VMU_TAG ) {
        ...
        [panel beginSheetModalForWindow: [self window] completionHandler:^(NSModalResponse result){
            if( result == NSModalResponseOK && [[panel URLs] count] > 0 ) {
                ...
                const char *selected_vmu = [[url path] UTF8String];
                int idx = vmulist_get_index_by_filename(vmu_filename);
                [sender selectItemWithTag: (FIRST_VMU_TAG+idx)];
                maple_device_class_t ndc = &vmu_class;
                set_gui_path(CONFIG_VMU_PATH, [[[url URLByDeletingLastPathComponent] path] UTF8String]);
                /* Reinvoke selection path with chosen file */
                maple_device_t cur = maple_get_device(port,slot);
                if( cur == NULL || cur->device_class != ndc ) {
                    /* Trigger handling after block */
                }
                /* Update and attach synchronously after sheet closes */
                maple_device_class_t new_dc = &vmu_class;
                maple_device_t new_dev = maple_new_device(new_dc->name);
                if( MAPLE_IS_VMU(new_dev) ) {
                    MAPLE_SET_VMU_NAME(new_dev, selected_vmu);
                }
                maple_attach_device(new_dev, port, slot);
            } else {
        ...
        [panel setDelegate: (id)valid];
        [panel setNameFieldStringValue:@"New.vmu"];
        [panel beginSheetModalForWindow: [self window] completionHandler:^(NSModalResponse result){
            if( result == NSModalResponseOK && [panel URL] != nil ) {
                NSURL *url = [panel URL];
                /* Validator already created the file */
                const char *created_vmu = [[url path] UTF8String];
                int idx = vmulist_get_index_by_filename(vmu_filename);
                [sender selectItemWithTag: (FIRST_VMU_TAG+idx)];
                set_gui_path(CONFIG_VMU_PATH, [[[url URLByDeletingLastPathComponent] path] UTF8String]);
                maple_device_class_t new_dc = &vmu_class;
                maple_device_t new_dev = maple_new_device(new_dc->name);
                if( MAPLE_IS_VMU(new_dev) ) {
                    MAPLE_SET_VMU_NAME(new_dev, created_vmu);
                }
                maple_attach_device(new_dev, port, slot);
            } else {
                setDevicePopupSelection( sender, current );
