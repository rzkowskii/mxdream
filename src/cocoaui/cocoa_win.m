...
- (id) init
{
    ...
    NSToolbarItem *load = [self createToolbarItem: @"LoadState" label: @"Load State..."
                           tooltip: @"Load an mxdream save state" icon: @"tb-load"
                           action: @selector(load_action:)];
    NSToolbarItem *save = [self createToolbarItem: @"SaveState" label: @"Save State..."
                           tooltip: @"Create an mxdream save state" icon: @"tb-save"
...
- (void)setIsGrabbed:(BOOL)grab
{
    if( grab != isGrabbed ) {
        ...
        if( isGrabbed ) {
            [NSCursor hide];
            // modern replacement: confine to center by warping
            CGPoint center = CGPointMake(CGRectGetMidX([[self screen] frame]), CGRectGetMidY([[self screen] frame]));
