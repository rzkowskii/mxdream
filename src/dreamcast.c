...
#ifdef __APPLE__
    {
        /* Raise QoS while emulation is running to improve responsiveness */
        typedef int (*set_qos_fn_t)(int, int);
        set_qos_fn_t set_qos = (set_qos_fn_t)dlsym(RTLD_DEFAULT, "pthread_set_qos_class_self_np");
        if( set_qos != NULL ) {
            /* QOS_CLASS_USER_INITIATED = 0x21 per headers; use literal to avoid SDK tie */
            set_qos(0x21, 0);
        }
    }
#endif
...
#ifdef __APPLE__
    {
        /* Restore to default QoS on exit from run loop */
        typedef int (*set_qos_fn_t)(int, int);
        set_qos_fn_t set_qos = (set_qos_fn_t)dlsym(RTLD_DEFAULT, "pthread_set_qos_class_self_np");
        if( set_qos != NULL ) {
            /* QOS_CLASS_DEFAULT = 0x15 per headers */
            set_qos(0x15, 0);
        }
    }
#endif
