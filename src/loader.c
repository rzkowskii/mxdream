                                            ...
                                            ERROR *err )
{
    ...
    char tmp[129];
    int name_len = snprintf( tmp, 129, "mxdream wrapped image: %s", filename );
    if( name_len < 128 )
...
//    bootstrap_update_crc(bootstrap);
    ...
    /* Without libisofs, we can't generate an ISO; return error */
    g_free(data);
    SET_ERROR( err, LX_ERR_UNHANDLED, "This build was compiled without libisofs; cannot wrap binary as ISO" );
    return disc;
    #endif
    if( disc != NULL ) {
