...
{
    print_version();
    printf( "Usage: mxdream %s [options] [disc-file] [save-state]\n\n", lxdream_full_version );
    ...
    printf( "   -v, --version          %s\n", _("Print the mxdream version string") );
...
{
    profiler_init();
    /* Apply conservative safe defaults unless explicitly overridden via env */
    ...
    if( getenv("LXDREAM_FSR_SHARP") == NULL ) setenv("LXDREAM_FSR_SHARP", "0", 0);
    ...
    /* On Apple Silicon, default to interpreter unless explicitly enabled via env or CLI */
#if defined(__APPLE__) && (defined(__aarch64__) || defined(__ARM64__) || defined(__arm64__))
    if( sh4_core != SH4_TRANSLATE ) {
        /* CLI overrode already */
    } else {
        const char *jitenv = getenv("LXDREAM_JIT");
        if( jitenv == NULL || atoi(jitenv) == 0 ) {
            sh4_core = SH4_INTERPRET;
            WARN("JIT disabled by default on Apple Silicon. Set LXDREAM_JIT=1 to try enabling.");
        }
    }
#endif
