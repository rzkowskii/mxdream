...
/**
 * Some embedded-focused shaders include GLES-only precision qualifiers
 * like 'precision mediump float;'. These are not valid in desktop GLSL
 * and will cause a compile failure. Since our shader generator does not
 * currently preprocess #ifdef HAVE_GLES2 in .glsl sources, strip those
 * lines defensively before compiling on desktop GL.
 */
static char *glsl_strip_gles_precision_lines( const char *src )
{
    ...
    while( *p ) {
        ...
        /* If line starts with 'precision' (GLES qualifier), drop it */
        static const char prec_kw[] = "precision";
        if( (size_t)(line_start + line_len - s) >= sizeof(prec_kw) - 1 &&
            strncmp(s, prec_kw, sizeof(prec_kw) - 1) == 0 ) {
            /* skip this line entirely */
        } else {
            /* copy line through */
            memcpy(q, line_start, line_len);
            q += line_len;
            if( nl ) { *q++ = '\n'; }
        }
...
static gl_shader_t glsl_create_fragment_shader( const char *source )
{
    ...
    /* Strip GLES precision qualifiers that desktop GLSL rejects */
    char *processed = glsl_strip_gles_precision_lines(source);
    const char *src_ptr = processed ? processed : source;
    glShaderSourceARB( shader, 1, &src_ptr, NULL );
    glCompileShaderARB(shader);
...
gboolean glsl_init( display_driver_t driver )
{
    ...
    if( isGLShaderSupported() && isGLMultitextureSupported() ) {
        const char *prewarm = getenv("LXDREAM_PREWARM");
        (void)prewarm; /* Compile/link always happens during init here (acts as prewarm) */
