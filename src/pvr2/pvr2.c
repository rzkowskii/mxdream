...
/* Present queue (newest-first) */
#define PRESENT_QUEUE_MAX 3
static render_buffer_t present_queue[PRESENT_QUEUE_MAX];
static int present_q_head = 0; /* next insert index */
static int present_q_count = 0;
static render_buffer_t last_presented = NULL;

/* Presentation pacing and gating */
static int present_divisor = 1; /* 1 = 60fps, 2 = 30fps */
static int present_counter = 0;
static gboolean frame_dirty = FALSE; /* Set when a new frame is ready to present */
static enum { PACE_AUTO = 0, PACE_60 = 60, PACE_30 = 30 } pace_mode = PACE_AUTO;
static gboolean pace_inited = FALSE;
static struct timeval last_present_tv = {0,0};
static uint32_t stats_last_ms = 0;
static uint32_t stats_frames = 0;
static uint32_t stats_presents = 0;
static uint32_t ema_present_ms = 16; /* simple EMA of present intervals */
/* Dynamic quality scaling (DQS) controller */
static gboolean dqs_enabled = FALSE;
static int dqs_min_percent = 50;
static int dqs_max_percent = 200;
static int dqs_step_percent = 10;
static int dqs_over_counter = 0;
static int dqs_under_counter = 0;
static gboolean dqs_inited = FALSE;
static int auto_over_counter = 0;  /* consecutive overruns (>20ms) */
static int auto_under_counter = 0; /* consecutive fast frames (<14ms when on 30) */
static int internal_scale_percent = 100; /* 50..200; default 100 */

/* Debug HUD state (compiled-in, env-gated) */
static int  dbg_enabled = 0;
static int  dbg_qdepth = 0, dbg_qcap = 0;
static int  dbg_list   = -1;
static char dbg_buf[128];

void pvr2_debug_init(void)
{
    const char* e = getenv("MXDREAM_DEBUG_HUD");
    dbg_enabled = (e && *e && *e != '0') ? 1 : 0;
}
...
const char* pvr2_debug_hud_string(void)
{
    ...
    const char* ls = (dbg_list>=0 && dbg_list<4)? names[dbg_list] : "--";
    snprintf(dbg_buf, sizeof(dbg_buf), "mxdream — Q:%d/%d  List:%s", dbg_qdepth, dbg_qcap, ls);
    return dbg_buf;
}

static inline uint32_t tv_delta_ms(struct timeval *a, struct timeval *b)
{
    return (uint32_t)((a->tv_sec - b->tv_sec) * 1000 + (a->tv_usec - b->tv_usec) / 1000);
}

static int present_queue_capacity(void)
{
    static int present_q_cap = 0;
    if( present_q_cap == 0 ) {
        const char *env = getenv("LXDREAM_PRESENT_Q");
        int cap = env ? atoi(env) : PRESENT_QUEUE_MAX;
        if( cap < 1 ) cap = 1;
        if( cap > PRESENT_QUEUE_MAX ) cap = PRESENT_QUEUE_MAX;
        present_q_cap = cap;
    }
    return present_q_cap;
}

static void present_queue_push(render_buffer_t buf)
{
    int cap = present_queue_capacity();
    if( present_q_count == cap ) {
        present_q_head = (present_q_head + cap - 1) % cap;
        present_q_count--;
    }
    present_queue[present_q_head] = buf;
    present_q_head = (present_q_head + 1) % cap;
    present_q_count++;
    /* Debug HUD: update queue metrics */
    extern void pvr2_debug_set_present_queue(int depth, int cap);
    pvr2_debug_set_present_queue(present_q_count, present_queue_capacity());
}

static render_buffer_t present_queue_peek_newest_ready(void)
{
    int cap = present_queue_capacity();
    if( present_q_count == 0 ) return NULL;
    int newest_idx = (present_q_head + cap - 1) % cap;
    render_buffer_t buf = present_queue[newest_idx];
    if( buf == NULL ) return NULL;
    if( buf->fence ) {
        GLenum w = glClientWaitSync(buf->fence, 0, 0);
        if( w == GL_TIMEOUT_EXPIRED ) {
            return NULL;
        }
    }
    present_q_head = newest_idx;
    present_q_count--;
    return buf;
}

/* Pick the last completed buffer (newest->oldest), never the newest-not-ready.
 * Maintains ring order by rotating the picked entry up to the newest slot so
 * we can reuse the same pop logic (update head and decrement count).
 */
static render_buffer_t present_queue_peek_last_completed(void)
{
    int cap = present_queue_capacity();
    if( present_q_count == 0 ) return NULL;
    /* newest is head-1, valid span length is present_q_count */
    int newest_idx = (present_q_head + cap - 1) % cap;
    for( int i = 0; i < present_q_count; i++ ) {
        int idx = (present_q_head - 1 - i + cap) % cap;
        render_buffer_t cand = present_queue[idx];
        if( cand == NULL ) continue;
        if( cand->fence ) {
            GLenum w = glClientWaitSync(cand->fence, 0, 0);
            if( w != GL_ALREADY_SIGNALED && w != GL_CONDITION_SATISFIED ) {
                continue; /* not ready, try older */
            }
        }
        /* If not already newest, rotate forward to newest position */
        int j = idx;
        while( j != newest_idx ) {
            int next = (j + 1) % cap;
            render_buffer_t tmp = present_queue[j];
            present_queue[j] = present_queue[next];
            present_queue[next] = tmp;
            j = next;
        }
        /* Pop newest (now our candidate) */
        render_buffer_t buf = present_queue[newest_idx];
        present_q_head = newest_idx;
        present_q_count--;
        return buf;
...
gboolean pvr2_frame_is_dirty(void)
{
    return frame_dirty;
}

void pvr2_mark_presented(void)
{
    frame_dirty = FALSE;
}

int pvr2_get_internal_scale_percent(void)
{
    return internal_scale_percent;
}

void pvr2_set_internal_scale_percent(int percent)
{
    if( percent < 50 ) percent = 50;
    if( percent > 200 ) percent = 200;
    internal_scale_percent = percent;
}

static void pvr2_update_pacing_after_present(uint32_t interval_ms)
{
    /* One-time init from env to allow forcing pacing mode: LXDREAM_PACE=auto|60|30 */
    if( !pace_inited ) {
        const char *pm = getenv("LXDREAM_PACE");
        if( pm ) {
            if( strcasecmp(pm, "60") == 0 ) pace_mode = PACE_60;
            else if( strcasecmp(pm, "30") == 0 ) pace_mode = PACE_30;
            else pace_mode = PACE_AUTO;
        }
        pace_inited = TRUE;
    }
    /* EMA smoothing (alpha ~ 0.1) to avoid flapping */
    ema_present_ms = (uint32_t)((ema_present_ms * 9 + interval_ms) / 10);
    if( pace_mode == PACE_60 ) { present_divisor = 1; return; }
    if( pace_mode == PACE_30 ) { present_divisor = 2; return; }
    /* Auto: use smoothed interval to decide step changes with hysteresis */
    if( present_divisor == 1 ) {
        if( ema_present_ms > 20 ) {
            if( ++auto_over_counter >= 8 ) { /* ~8 consecutive overruns */
                present_divisor = 2;
                auto_over_counter = 0;
                auto_under_counter = 0;
            }
        } else {
            auto_over_counter = 0;
        }
    } else { /* currently at 30fps */
        if( ema_present_ms < 16 ) {
            if( ++auto_under_counter >= 60 ) { /* ~1s of headroom */
                present_divisor = 1;
                auto_under_counter = 0;
                auto_over_counter = 0;
            }
    ...
    /* Dynamic quality scaling: adjust internal render scale based on sustained frame time */
    if( !dqs_inited ) {
        const char *en = getenv("LXDREAM_DQS");
        dqs_enabled = (en && atoi(en) != 0) ? TRUE : FALSE;
        const char *mn = getenv("LXDREAM_DQS_MIN"); if( mn ) { int v=atoi(mn); if(v>=25&&v<=100)dqs_min_percent=v; }
        const char *mx = getenv("LXDREAM_DQS_MAX"); if( mx ) { int v=atoi(mx); if(v>=100&&v<=300)dqs_max_percent=v; }
        const char *st = getenv("LXDREAM_DQS_STEP"); if( st ) { int v=atoi(st); if(v>=5&&v<=50)dqs_step_percent=v; }
        dqs_inited = TRUE;
    }
    if( dqs_enabled ) {
        /* Targets depend on pace: 60fps -> ~16ms, 30fps -> ~33ms */
        uint32_t target_ms = (present_divisor == 1) ? 16U : 33U;
        /* Hysteresis bands: scale down if > target+4ms sustained; up if < target-4ms sustained */
        uint32_t high = target_ms + 4U;
        uint32_t low  = (target_ms > 4U) ? (target_ms - 4U) : target_ms;
        int cur = pvr2_get_internal_scale_percent();
        if( ema_present_ms > high ) {
            if( ++dqs_over_counter >= 20 ) { /* ~20 frames sustained */
                int next = cur - dqs_step_percent;
                if( next < dqs_min_percent ) next = dqs_min_percent;
                if( next != cur ) {
                    pvr2_set_internal_scale_percent(next);
                }
                dqs_over_counter = 0;
                dqs_under_counter = 0;
            }
        } else if( ema_present_ms < low ) {
            if( ++dqs_under_counter >= 90 ) { /* ~1.5s sustained headroom */
                int next = cur + dqs_step_percent;
                if( next > dqs_max_percent ) next = dqs_max_percent;
                if( next != cur ) {
                    pvr2_set_internal_scale_percent(next);
                }
                dqs_under_counter = 0;
            ...
            /* inside band: decay counters to avoid stale triggers */
            if( dqs_over_counter > 0 ) dqs_over_counter--;
            if( dqs_under_counter > 0 ) dqs_under_counter--;
        }
    }
}
...
void pvr2_draw_frame()
{
    ...
    /* Gate presentation for adaptive pacing and only when a new frame is ready */
    if( ++present_counter < present_divisor ) {
        return; /* skip this present to pace down */
    }
    present_counter = 0;

    if( !frame_dirty ) {
        return; /* nothing new to present */
    }

    struct timeval nowtv; gettimeofday(&nowtv, NULL);
    uint32_t interval_ms = (last_present_tv.tv_sec==0 && last_present_tv.tv_usec==0) ? 0 : tv_delta_ms(&nowtv, &last_present_tv);
    last_present_tv = nowtv;
    if( interval_ms > 0 ) {
        pvr2_update_pacing_after_present(interval_ms);
    }

    os_signpost_id_t sid_present = profiler_begin("present");
    /* Prefer the newest buffer that has actually completed on the GPU. */
    render_buffer_t to_present = present_queue_peek_last_completed();
    if( to_present != NULL ) {
        displayed_render_buffer = to_present;
        last_presented = to_present;
    }
    if( displayed_render_buffer == NULL ) {
        display_driver->display_blank(displayed_border_colour);
    } else {
        render_buffer_t buf = (to_present != NULL) ? to_present : (last_presented ? last_presented : displayed_render_buffer);
        display_driver->display_render_buffer(buf);
    }
    frame_dirty = FALSE;
    profiler_end("present", sid_present);

    /* Rolling stats every ~2s to stderr */
    stats_frames++;
    stats_presents++;
    uint32_t now_ms = (uint32_t)(nowtv.tv_sec*1000 + nowtv.tv_usec/1000);
    if( stats_last_ms == 0 ) {
        stats_last_ms = now_ms;
    } else if( now_ms - stats_last_ms >= 2000 ) {
        extern uint64_t pvr2_texcache_bytes_uploaded_consume(void);
        uint64_t up_bytes = pvr2_texcache_bytes_uploaded_consume();
        fprintf(stderr, "[mxdream] fps=%u presents=%u pace=%s divisor=%d qdepth=%d/%d uploads=%lluKB/2s\n",
                stats_frames, stats_presents,
                (pace_mode==PACE_60?"60":(pace_mode==PACE_30?"30":"auto")),
                present_divisor,
                present_q_count, present_queue_capacity(),
                (unsigned long long)(up_bytes/1024ULL));
        stats_frames = 0;
        stats_presents = 0;
        stats_last_ms = now_ms;
    }
