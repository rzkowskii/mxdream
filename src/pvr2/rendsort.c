...
{
    const char *sort_en = getenv("LXDREAM_SORT_OPAQUE");
    if( sort_en && atoi(sort_en) == 0 ) {
        /* Conservative fallback: use original order without sorting */
        gl_render_tilelist(tile_entry, render_mode!=RENDER_ZONLY);
        return;
    }
