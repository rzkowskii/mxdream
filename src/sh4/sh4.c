...
void sh4_init(void)
{
    DEBUG("sh4_init: start");
    register_io_regions( mmio_list_sh4mmio );
    DEBUG("sh4_init: IO regions registered");
    register_event_callback( EVENT_ENDTIMESLICE, sh4_dummy_event );
    DEBUG("sh4_init: event callback registered");
    MMU_init();
    DEBUG("sh4_init: MMU_init ok");
    TMU_init();
    DEBUG("sh4_init: TMU_init ok");
#ifdef SH4_TRANSLATOR
    if( sh4_use_translator ) {
        xlat_cache_init();
        DEBUG("sh4_init: xlat_cache_init ok");
    }
#endif
    sh4_poweron_reset();
    DEBUG("sh4_init: poweron_reset ok");
...
void sh4_poweron_reset(void)
{
    ...
    DEBUG("SH4 PC set to %08X, SR=%08X", sh4r.pc, sh4r.sr );
